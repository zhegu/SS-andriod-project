...
local function ButtonDownSound(effectType)
	...
	elseif effectType == BUTTON_SOUND_CLICK then
		--普通点击音效
		AudioManager.playGameSound(AudioManager.TableSound.CLICK, false, AudioManager.SOUND);
	elseif effectType == BUTTON_SOUND_BACK then
		--返回点击音效
		AudioManager.playGameSound(AudioManager.TableSound.BACK, false, AudioManager.SOUND);
	else
		AudioManager.playGameSound(AudioManager.TableSound.CLICK, false, AudioManager.SOUND);
	end
end
...
local function zoomInAnimation(component, event, callbackFunction)
	if event == PUSH_DOWN then
		--按下
		GamePub.buttonActionPress(component, 1.05, true, callbackFunction, event)
	elseif event == RELEASE_UP then
		--抬起
		GamePub.buttonActionUp(component, 0.95, callbackFunction, event)
	elseif event == CANCEL_UP then
		--取消
		GamePub.buttonActionUp(component, 0.95, nil)
	end
end
...
local function zoomOutAnimation(component, event, callbackFunction)
	if event == PUSH_DOWN then
		--按下
		GamePub.buttonActionPress(component, 0.95, true, callbackFunction, event)
	elseif event == RELEASE_UP then
		--抬起
		GamePub.buttonActionUp(component, 1.2, callbackFunction, event)
	elseif event == CANCEL_UP then
		--取消
		GamePub.buttonActionUp(component, 1.2, nil, nil)
	end
end
...
local function zoomInAnimationUpExecute(component, event, callbackFunction)
	...
	elseif event == RELEASE_UP then
		--抬起
		GamePub.buttonActionUp(component, 0.95, nil, nil)
