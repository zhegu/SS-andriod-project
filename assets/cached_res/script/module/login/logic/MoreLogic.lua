...
function refreshData()
	NickNameTable = profileCommon.MoreUser.getUserListFromIMEI();
	local j = 0;
...
function callback_Img_DeleteTopUser(component)
	...
	elseif component == RELEASE_UP then
		...
		if #NickNameTable > 2 then
			...
			mvcEngine.createModule(GUI_NIUNIUDOUBLEBUTTONCONFIRM)
			NiuNiuDoubleButtonConfirmLogic.showDeleteUserView(NiuNiuDoubleButtonConfirmLogic.getTypeTable().deleteUser, 1, username)
		else
...
function callback_Img_DeleteMiddleUser(component)
	...
	elseif component == RELEASE_UP then
		...
		if #NickNameTable > 2 then
			...
			mvcEngine.createModule(GUI_NIUNIUDOUBLEBUTTONCONFIRM)
			NiuNiuDoubleButtonConfirmLogic.showDeleteUserView(NiuNiuDoubleButtonConfirmLogic.getTypeTable().deleteUser, 2, username)
		else
			Common.showToast("你好，你目前的专属账号太少了，删除之后不易找回。", 2);
		end
	elseif component == CANCEL_UP then
...
function callback_Img_DeleteBottomUser(component)
	...
	elseif component == RELEASE_UP then
		...
		if #NickNameTable > 2 then
			...
			mvcEngine.createModule(GUI_NIUNIUDOUBLEBUTTONCONFIRM)
			NiuNiuDoubleButtonConfirmLogic.showDeleteUserView(NiuNiuDoubleButtonConfirmLogic.getTypeTable().deleteUser, 3, username)
		else
			Common.showToast("你好，你目前的专属账号太少了，删除之后不易找回。", 2);
		end
...
function touchUserInfoLogic(username)
	if CurViewTag == ViewTag.LOGIN then
		profileCommon.MoreUser.setLoginWithMore(username);
