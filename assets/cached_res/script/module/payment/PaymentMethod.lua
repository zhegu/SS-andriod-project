...
function callPayment(PaymentTable, PayTypeID, GiftID, isExchange, position)
	...
	profilePay.PaymentResult.setPaymentTable(PaymentTable)

	if PayTypeID == profilePay.PayChannelData.RECHARGE_91 then
		--91
	...
	elseif PayTypeID == profilePay.PayChannelData.IAP_PAY then
		--IAP
		local function callIAPPayGetOrderInfo(parameters)
			...
			--appstore审核版本，上传appleId
			Common.log("PaymentTable.price === "..PaymentTable.price)
	...
	elseif PayTypeID == profilePay.PayChannelData.HAIMA_PAY then
		--海马
	...
	elseif PayTypeID == profilePay.PayChannelData.ALI_PAY then
		--支付宝
	...
	elseif PayTypeID == profilePay.PayChannelData.WEIXIN_PAY then
		--微信
	...
	elseif PayTypeID == profilePay.PayChannelData.UNION_PAY then
	--银联
	elseif PayTypeID == profilePay.PayChannelData.RECHARGE_CARD_PAY then
		--充值卡
		Common.log("PaymentTable.goodsName = " .. PaymentTable.goodsName)-- 商品名称
		Common.log("PaymentTable.goodsDetail = " .. PaymentTable.goodsDetail)-- 商品的具体描述
		Common.log("PaymentTable.goodsPriceDetail = " .. PaymentTable.goodsPriceDetail)-- 本次支付的总费
	...
	elseif PayTypeID == profilePay.PayChannelData.MM_PAY_V2 then
		--MM
		Common.log("PaymentTable.goodsName = " .. PaymentTable.goodsName)-- 商品名称
		Common.log("PaymentTable.goodsDetail = " .. PaymentTable.goodsDetail)-- 商品的具体描述
		Common.log("PaymentTable.goodsPriceDetail = " .. PaymentTable.goodsPriceDetail)-- 本次支付的总费
		...
		--	elseif PayTypeID == profilePay.PayChannelData.CTESTORE_PAY then
		--		--电信
		--		Common.log("PaymentTable.goodsName = " .. PaymentTable.goodsName)-- 商品名称
		--		Common.log("PaymentTable.goodsDetail = " .. PaymentTable.goodsDetail)-- 商品的具体描述
		--		Common.log("PaymentTable.goodsPriceDetail = " .. PaymentTable.goodsPriceDetail)-- 本次支付的总费
		...
		--		end
		--		sendMANAGERID_V3_RECHARGE(PaymentTable, PaymentInformation, PayTypeID, position)
	elseif PayTypeID == profilePay.PayChannelData.RECHARGE_WOJIA then
		--同趣联通WO+
		Common.log("PaymentTable.goodsName = " .. PaymentTable.goodsName)-- 商品名称
		Common.log("PaymentTable.goodsDetail = " .. PaymentTable.goodsDetail)-- 商品的具体描述
		Common.log("PaymentTable.goodsPriceDetail = " .. PaymentTable.goodsPriceDetail)-- 本次支付的总费
		...
		sendMANAGERID_V3_RECHARGE(PaymentTable, PaymentInformation, PayTypeID, position)
	elseif PayTypeID == profilePay.PayChannelData.NEW_UNION_PAY then
		--新银联
		PaymentInformation = {}
		PaymentInformation.giftID = GiftID
		if isExchange then
		...
		sendMANAGERID_V3_RECHARGE(PaymentTable, PaymentInformation, PayTypeID, position)
		--	elseif PayTypeID == profilePay.PayChannelData.CM_PAY then
		--		--手机钱包
		--		Common.log("PaymentTable.goodsName = " .. PaymentTable.goodsName)-- 商品名称
		--		Common.log("PaymentTable.goodsDetail = " .. PaymentTable.goodsDetail)-- 商品的具体描述
		--		Common.log("PaymentTable.goodsPriceDetail = " .. PaymentTable.goodsPriceDetail)-- 本次支付的总费
		...
		--		end
		--		sendMANAGERID_V3_RECHARGE(PaymentTable, PaymentInformation, PayTypeID, position)
	elseif PayTypeID == profilePay.PayChannelData.SMS_ONLINE then
		--移动短代支付
		Common.log("移动短代支付")
		Common.log("PaymentTable.goodsName = " .. PaymentTable.goodsName)-- 商品名称
		Common.log("PaymentTable.goodsDetail = " .. PaymentTable.goodsDetail)-- 商品的具体描述
		...
		sendMANAGERID_V3_RECHARGE(PaymentTable, PaymentInformation, PayTypeID, position)
	elseif PayTypeID == profilePay.PayChannelData.SMS_UNICOM then
		--联通沃支付
		Common.log("PaymentTable.goodsName = " .. PaymentTable.goodsName)-- 商品名称
		Common.log("PaymentTable.goodsDetail = " .. PaymentTable.goodsDetail)-- 商品的具体描述
		Common.log("PaymentTable.goodsPriceDetail = " .. PaymentTable.goodsPriceDetail)-- 本次支付的总费
		...
		--
		--		end
	elseif PayTypeID == profilePay.PayChannelData.HUAJIAN_DIANXIN_PAY then
		--电信短代支付
		Common.log("PaymentTable.goodsName = " .. PaymentTable.goodsName)-- 商品名称
		Common.log("PaymentTable.goodsDetail = " .. PaymentTable.goodsDetail)-- 商品的具体描述
		Common.log("PaymentTable.goodsPriceDetail = " .. PaymentTable.goodsPriceDetail)-- 本次支付的总费
		...
		sendMANAGERID_V3_RECHARGE(PaymentTable, PaymentInformation, PayTypeID, position)
	elseif PayTypeID == profilePay.PayChannelData.HUAJIAN_LIANTONG_PAY then
		--联通短代支付
		Common.log("PaymentTable.goodsName = " .. PaymentTable.goodsName)-- 商品名称
		Common.log("PaymentTable.goodsDetail = " .. PaymentTable.goodsDetail)-- 商品的具体描述
		Common.log("PaymentTable.goodsPriceDetail = " .. PaymentTable.goodsPriceDetail)-- 本次支付的总费
		...
		sendMANAGERID_V3_RECHARGE(PaymentTable, PaymentInformation, PayTypeID, position)
	elseif PayTypeID == profilePay.PayChannelData.EPAY then
		--宜支付
		Common.log("PaymentTable.goodsName = " .. PaymentTable.goodsName)-- 商品名称
		Common.log("PaymentTable.goodsDetail = " .. PaymentTable.goodsDetail)-- 商品的具体描述
		Common.log("PaymentTable.goodsPriceDetail = " .. PaymentTable.goodsPriceDetail)-- 本次支付的总费
		...
		sendMANAGERID_V3_RECHARGE(PaymentTable, PaymentInformation, PayTypeID, position)
	elseif PayTypeID == profilePay.PayChannelData.YINBEIKEPAY_CMCC then
		--银贝壳移动
		Common.log("PaymentTable.goodsName = " .. PaymentTable.goodsName)-- 商品名称
		Common.log("PaymentTable.goodsDetail = " .. PaymentTable.goodsDetail)-- 商品的具体描述
		Common.log("PaymentTable.goodsPriceDetail = " .. PaymentTable.goodsPriceDetail)-- 本次支付的总费
		...
		sendMANAGERID_V3_RECHARGE(PaymentTable, PaymentInformation, PayTypeID, position)
	elseif PayTypeID == profilePay.PayChannelData.YINBEIKEPAY_UNI then
		--银贝壳联通
		Common.log("PaymentTable.goodsName = " .. PaymentTable.goodsName)-- 商品名称
		Common.log("PaymentTable.goodsDetail = " .. PaymentTable.goodsDetail)-- 商品的具体描述
		Common.log("PaymentTable.goodsPriceDetail = " .. PaymentTable.goodsPriceDetail)-- 本次支付的总费
		...
		sendMANAGERID_V3_RECHARGE(PaymentTable, PaymentInformation, PayTypeID, position)
	elseif PayTypeID == profilePay.PayChannelData.YINBEIKEPAY_CT then
		--银贝壳电信
		Common.log("PaymentTable.goodsName = " .. PaymentTable.goodsName)-- 商品名称
		Common.log("PaymentTable.goodsDetail = " .. PaymentTable.goodsDetail)-- 商品的具体描述
		Common.log("PaymentTable.goodsPriceDetail = " .. PaymentTable.goodsPriceDetail)-- 本次支付的总费
		...
		sendMANAGERID_V3_RECHARGE(PaymentTable, PaymentInformation, PayTypeID, position)
	elseif PayTypeID == profilePay.PayChannelData.HONGRUAN_SDK_CMCC then
		--红软移动
		...
		sendMANAGERID_V3_RECHARGE(PaymentTable, PaymentInformation, PayTypeID, position)
	elseif PayTypeID == profilePay.PayChannelData.HONGRUAN_SDK_UNICOM then
		--红软联通
		Common.log("PaymentTable.goodsName = " .. PaymentTable.goodsName)-- 商品名称
		Common.log("PaymentTable.goodsDetail = " .. PaymentTable.goodsDetail)-- 商品的具体描述
		Common.log("PaymentTable.goodsPriceDetail = " .. PaymentTable.goodsPriceDetail)-- 本次支付的总费
		...
		sendMANAGERID_V3_RECHARGE(PaymentTable, PaymentInformation, PayTypeID, position)
	elseif PayTypeID == profilePay.PayChannelData.HongRuanTableCT then
		--红软电信
		Common.log("PaymentTable.goodsName = " .. PaymentTable.goodsName)-- 商品名称
		Common.log("PaymentTable.goodsDetail = " .. PaymentTable.goodsDetail)-- 商品的具体描述
		Common.log("PaymentTable.goodsPriceDetail = " .. PaymentTable.goodsPriceDetail)-- 本次支付的总费
