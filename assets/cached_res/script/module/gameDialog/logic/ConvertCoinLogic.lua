...
function createView()
	...
	initView()
	GamePub.showDialogAmin(Panel_31)
end
...
function initView()
	...
	convert_coin = cocostudio.getUILabelAtlas(view, "convert_coin")
	convert_num = cocostudio.getUILabelAtlas(view, "txt_num")
	convert_num:setVisible(false);
	convert_yuanbao = cocostudio.getUILabelAtlas(view, "txt_convertyuanbao")
	...
	Panel_31 = cocostudio.getUIPanel(view, "Panel_31");
...
--		convert_num:setStringValue(1)
--	else
		convert_num:setStringValue(myyuanbao)
--	end
	convert_yuanbao:setStringValue(Price*convert_num:getStringValue())
	convert_coin:setStringValue(convert_coinvalue*convert_num:getStringValue())
...
function seriesChangeNum(changType)
	...
	if tonumber(convertnum) ~= nil and isNumber(convertnum) then --如果兑换数量是数字或非空
			...
			function()
				...
				array1:addObject(CCCallFunc:create(
					function ()
						...
						convert_num:setStringValue(convertnum)
						convert_coin:setStringValue(convert_coinvalue*convertnum)
						convert_yuanbao:setStringValue(convertnum*Price)
					end
				))
		...
		Common.showToast("您的元宝不足，无法兑换金币", 2)
	end
end
	...
	elseif component == RELEASE_UP then
		...
		if tonumber(convertnum) ~= nil and isNumber(convertnum) then
			...
			if(convertnum>1) then
				...
				convert_num:setStringValue(1)
				convert_yuanbao:setStringValue(Price)
				convert_coin:setStringValue(convert_coinvalue*1)
			end
		else
			Common.showToast("您的元宝不足，无法兑换金币", 2)
		end
	...
	elseif component == RELEASE_UP then
		...
		if isNumber(GoodsNum) then
			if profile.User.getSelfYuanBao() >=  Price * GoodsNum then
				...
				GiftGuideConfig.showPayGuide(QuickPay.Pay_Guide_need_yuanbao_GuideTypeID, num ,RechargeGuidePositionID.ShopPositionA)
...
function callback_txt_num()
	convert_yuanbao:setStringValue(convert_num:getStringValue())
...
function callbackConvertNumInput(valuetable)
	...
	if tonumber(value) > 0 then
		convert_num:setStringValue(value)
		convert_yuanbao:setStringValue(value)
		convert_coin:setStringValue(convert_coinvalue*convert_num:getStringValue())
	end
end
...
function resultBuyGoods()
	local resultData = profileCommon.BuyGoods.getBuyGoodsData()
...
--		ImageToast.createView(nil,Common.getResourcePath("DN_HALL_getreward_bg_cover_gold.png"),"兑换金币成功",resultMsg,2)
	end
...
--关闭
function close()
	GamePub.closeDialogAmin(Panel_31,closePanel)
end
function closePanel()
	mvcEngine.destroyModule(GUI_CONVERTCOIN)
end
