...
public class MatchAlarmBroadCast extends BroadcastReceiver {
		...
		if (intent.getIntExtra("NotificationPushMessage", 0) == 1) {
			...
			if (intent.getIntExtra("ActionID", 0) == 2) {
				...
				if (day != lastDay) {
					...
					Intent intentPendingIntent = new Intent(context, TQLuaAndroidConsole.getLoadActivityClass());
					// 获取PendingIntent,点击时发送该Intent
					PendingIntent pIntent = PendingIntent.getActivity(context, 0, intentPendingIntent, 0);
					notification.setLatestEventInfo(context, intent.getStringExtra("Title"), intent.getStringExtra("Content"), pIntent);
					notificationManager.notify(0, notification);
				} else {
					// Pub.LOG("onReceive ================ day = lastDay");
				}
			...
			if (intent.getIntExtra("ActionID", 0) == 1) {
				...
				// TQLuaAndroidConsole.getGameSceneInstance().getSystemService(Context.ALARM_SERVICE);
				...
				// PendingIntent.getBroadcast(TQLuaAndroidConsole.getGameSceneInstance(),
				// intent.getIntExtra("NotificationID", 0) + 5000, intent_re,
				// 0);
				PendingIntent mAlarmSender = PendingIntent.getBroadcast(context, intent.getIntExtra("NotificationID", 0) + 5000, intent_re, 0);
	...
	private boolean isTongquAppRunning(Context context) {
		...
		for (RunningAppProcessInfo ra : run) {
			...
			if (ra.processName.indexOf("com.shuashuaent.client") >= 0 && ra.processName.indexOf("com.shuashuaent.client." + TQLuaAndroidConsole.APP_NAME) == -1) {
				return true;
			}
		}
		return false;
	...
	private boolean isTopActivity(Context context, String sActivityName) {
		// "com.shuashuaent.client." + Const.APP_NAME + ".LordActivity";
		ActivityManager activityManager = (ActivityManager) context.getSystemService(Context.ACTIVITY_SERVICE);
	...
	public static void addAlarm(int nGameID, int nMatchID, long nAlertTime, String sMatchInstanceID, String matchTitle) {
		...
		for (int i = 1; i < 3; i++) {
			...
			b.putInt("currgameid", TQLuaAndroidConsole.GAME_ID);
			b.putString("apkname", "com.shuashuaent.client." + TQLuaAndroidConsole.APP_NAME);
