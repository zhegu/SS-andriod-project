...
public class DownloadThread extends Thread {
	...
	public void run() {
		...
		try {
				...
				while (!mDownloadInfo.isStop()) {// 若没有停止
					if (!mDownloadInfo.isPause()) {// 没有暂停（暂停时线程空转）
						if ((offset = is.read(buffer, 0, BUFFER_SIZE)) != -1) {
							...
							if (mDownloadInfo.getGamePackage() != null && mDownloadInfo.getLuaCallback() > 0) {
								Downloader.callbackLuaProgress(mDownloadInfo.getLuaCallback(), mDownloadInfo.getGamePackage(), mDownloadInfo.getTotalSize(), mDownloadInfo.getDownloadSize());
							} else {
			...
			Pub.LOG("finally ------------------");
			Pub.LOG("finally ---------------mDownloadInfo.getDownloadSize()---" + mDownloadInfo.getDownloadSize());
			Pub.LOG("finally ---------------mDownloadInfo.getTotalSize()---" + mDownloadInfo.getTotalSize());
			if (mDownloadInfo.getDownloadSize() == mDownloadInfo.getTotalSize() && mDownloadInfo.getTotalSize() > 0) {// 下载完成
				Pub.LOG("finally ------------------Success");
				if (mDownloadInfo.getGamePackage() != null && mDownloadInfo.getLuaCallback() > 0) {
					Downloader.callbackLuaProgress(mDownloadInfo.getLuaCallback(), mDownloadInfo.getGamePackage(), mDownloadInfo.getTotalSize(), mDownloadInfo.getDownloadSize());
				}
